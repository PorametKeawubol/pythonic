def create_triangle_dict(n):
    triangle_dict = {}
    vertices = 1
    triangles = 0
    increment_vertices = 2
    increment_triangles = 1

    while vertices <= n:
        triangle_dict[vertices] = triangles
        vertices += increment_vertices
        increment_vertices += 1
        triangles += increment_triangles
        increment_triangles += 2

    return triangle_dict

def max_triangles(n, triangle_dict):
    remaining_points = n
    max_triangles = 0
    
    # Sort the keys in descending order
    sorted_keys = sorted(triangle_dict.keys(), reverse=True)
    
    for key in sorted_keys:
        while remaining_points >= key:
            remaining_points -= key
            max_triangles += triangle_dict[key]
    
    return max_triangles

# กำหนดค่า n
n =10**9
  # คุณสามารถเปลี่ยนค่านี้ได้ตามต้องการ

# สร้าง dictionary
triangle_dict = create_triangle_dict(n)

# หาจำนวนสามเหลี่ยมมากสุด
max_triangles_count = max_triangles(n, triangle_dict)

# แสดงผลลัพธ์
print(f"จำนวนสามเหลี่ยมมากสุดที่สามารถสร้างได้จาก {n} จุด คือ {max_triangles_count}")
